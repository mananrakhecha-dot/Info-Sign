...
/**
 * Platform Signing Keys — a dedicated leaf certificate issued by the
 ...
 * It is NOT the CA certificate (which has keyCertSign + cRLSign and is
 * rejected by Adobe Acrobat for document signing purposes).
 *
 * Same shape as CAKeys — reused for consistency.
 */
export type PlatformSigningKeys = CAKeys;
...
export function saveEncryptedPem(
  filename: string,
  pem: string,
  passphrase: string,
): void {
...
export function loadEncryptedPem(
  filename: string,
  passphrase: string,
): string | null {
...
export function platformSigningKeysExist(): boolean {
  return (
    fs.existsSync(path.join(CA_DIR, "platform-signing.enc")) &&
    fs.existsSync(path.join(CA_DIR, "platform-signing-cert.pem"))
  );
}

// ── Intermediate CA singleton ─────────────────────────────────────────────────
...
// ── Platform Signing Keys singleton ──────────────────────────────────────────

let _platformSigningKeys: PlatformSigningKeys | null = null;

export function setPlatformSigningKeys(keys: PlatformSigningKeys): void {
  _platformSigningKeys = keys;
}

export function getPlatformSigningKeys(): PlatformSigningKeys {
  if (!_platformSigningKeys)
    throw new Error("Platform signing keys not loaded");
  return _platformSigningKeys;
}
