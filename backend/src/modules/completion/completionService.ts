...
export async function generateCertificateOfCompletion(
  ...
  page.drawText("Digital Document Signing Platform", {
    x: margin,
    y: height - 70,
    size: 11,
    font,
  ...
  drawField(
    ...
    boldFont,
    "Sender:",
    `${envelope.sender_name} <${envelope.sender_email}>`,
    margin,
    y,
    darkGray,
    lightGray,
  ...
  drawField(
    ...
    boldFont,
    "Completed At:",
    new Date(envelope.completed_at).toUTCString(),
    margin,
    y,
    darkGray,
    lightGray,
  ...
  for (const r of recipients) {
    const signerEvents = events.filter(
      (e: any) => e.recipient_email === r.user_email,
    );
    const viewedEvent = signerEvents.find(
      (e: any) => e.event_type === "signing_link_opened",
    );
    const signedEvent = signerEvents.find(
      (e: any) => e.event_type === "signed",
    );
    const idEvent = signerEvents.find(
      (e: any) => e.event_type === "identity_verified",
    );
    ...
    drawField(
      ...
      y - 56,
      darkGray,
      r.status === "SIGNED" ? green : lightGray,
    );
    if (viewedEvent)
      drawField(
        page,
        ...
        boldFont,
        "Viewed:",
        new Date(viewedEvent.created_at).toUTCString(),
        margin + 8,
        y - 72,
        darkGray,
        lightGray,
    ...
    if (signedEvent)
      drawField(
        ...
        boldFont,
        "Signed:",
        new Date(signedEvent.created_at).toUTCString(),
        margin + 220,
        y - 72,
        darkGray,
        green,
...
function drawField(
  ...
  page.drawText(value.substring(0, 70), {
    x: x + 90,
    y,
    size: 8,
    font,
...
export async function getVerificationData(envelopeId: string): Promise<any> {
  ...
  const { rows: docs } = await query<any>(
    "SELECT file_name, sha256_hash FROM envelope_documents WHERE envelope_id=$1",
    [envelopeId],
  );
  const { rows: recipients } = await query<any>(
  ...
  return {
    ...
    document: docs[0]
      ? { name: docs[0].file_name, sha256Hash: docs[0].sha256_hash }
      : null,
