...
export async function generateCompletionCertificate(
  ...
  if (doc) {
    ...
    for (const f of fields) {
      if (!recipientFields[f.recipient_id])
        recipientFields[f.recipient_id] = [];
  ...
  page.drawText(envelope.subject || "Untitled Envelope", {
    x: MARGIN,
    y: PAGE_H - 52,
    size: 11,
    font,
  ...
  page.drawText(formatUtc(new Date()), {
    x: MARGIN + 75,
    y,
    size: 8,
    font,
  ...
  page.drawText(formatUtc(envelope.completed_at), {
    x: MARGIN + 75,
    y,
    size: 8,
    font,
  ...
  page.drawText("Original SHA-256:", {
    x: MARGIN,
    y,
    size: 8,
    font: boldFont,
  ...
  y -= LINE - 2;
  page.drawText(doc?.sha256_hash || "N/A", {
    x: MARGIN + 10,
    y,
    size: 7,
    font,
  ...
  y -= LINE;
  page.drawText("Signed document SHA-256:", {
    x: MARGIN,
    y,
    size: 8,
    font: boldFont,
  ...
  y -= LINE;
  page.drawText("Integrity status:", {
    x: MARGIN,
    y,
    size: 8,
    font: boldFont,
  ...
  const sigLines: [string, string][] = [
    ...
    [
      ...
      "Self-signed (internal CA). PDF readers will show",
    ],
  ...
  for (const r of recipients) {
    ...
    page.drawText(r.full_name || r.user_full_name || "Unknown", {
      x: MARGIN + 8,
      y: y - 12,
      size: 9,
      font: boldFont,
    ...
    page.drawText(formatUtc(r.signed_at), {
      x: MARGIN + 60,
      y: y - 38,
      size: 8,
      font,
    ...
    page.drawText(r.signing_ip || r.ip_address || "N/A", {
      x: col2 + 58,
      y: y - 38,
      size: 8,
      font,
    ...
    const statusColor =
      r.status === "SIGNED" ? green : r.status === "DECLINED" ? red : gray;
  ...
  drawHRule(y);
  y -= LINE;
  page.drawText("This certificate was generated automatically.", {
    x: MARGIN,
    y,
    size: 8,
    font,
  ...
  y -= LINE - 2;
  page.drawText(
    "Verify document integrity by recomputing the SHA-256 hash of the signed PDF.",
    { x: MARGIN, y, size: 8, font, color: gray },
  );
  y -= LINE - 2;
  page.drawText(
    "Document will show a valid digital signature in any PDF reader.",
    { x: MARGIN, y, size: 8, font, color: gray },
  );
  ...
  await query(
    ...
    [
      envelopeId,
      `certificate-${envelopeId}.pdf`,
      certPath,
      computeSHA256(pdfBuffer),
      1,
    ],
  );
