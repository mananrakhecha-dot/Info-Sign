...
import { cocQueue } from "../../jobs/queues";
...
export async function completeSigningCeremony(
  ...
  if (!canSign(user?.identity_level, recipient.auth_required)) {
    throw new AppError(
      `Identity level ${recipient.auth_required} required to sign`,
      403,
    );
  }
  ...
  // Step 2 — Fetch document + signing_mode + original hash
  // signing_mode (SINGLE|MULTI) was set once in sendEnvelope() and drives
  // whether we apply PKCS#7 now (SINGLE) or visual-stamp-only (MULTI).
  // original_sha256_hash is the hash of the original unsigned PDF — never
  // overwritten — used for tamper detection across multiple signers.
  const { rows: docs } = await query<any>(
    `SELECT ed.*, e.signing_mode
  ...
  // Tamper check — compare against sha256_hash (the hash after the last
  // legitimate write: either upload time or previous signer's visual stamp).
  // This detects external tampering between signing ceremonies.
  // original_sha256_hash is preserved for audit trail purposes only.
  ...
  if (!user?.cert_pem || !user?.encrypted_private_key) {
    throw new AppError(
      "Signer certificate not found. Please complete identity verification.",
      403,
    );
  }
  ...
  // SINGLE: recipient's own PKCS#7 applied now — one valid signature in Acrobat.
  // MULTI:  visual stamp only — Platform CA applies the final PKCS#7 after all
  //         recipients have signed (in the cocQueue worker).
  const signingMode: string = docs[0].signing_mode ?? "SINGLE";
  let signedPdfBuffer: Buffer;

  if (signingMode === "SINGLE") {
    signedPdfBuffer = await embedSignatureIntoPDF(
      docBuffer,
      user.cert_pem,
      privateKeyPem,
      fieldsForEmbed,
    ...
    // MULTI — draw visual stamp only, no PKCS#7
    signedPdfBuffer = await drawVisualSignatureOnly(
      docBuffer,
      user.cert_pem,
      fieldsForEmbed,
      appearance,
  ...
  // Update sha256_hash — tracks the current state of the file after each
  // signer. original_sha256_hash is intentionally NOT updated here.
  await query("UPDATE envelope_documents SET sha256_hash=$1 WHERE id=$2", [
    newHash,
    docs[0].id,
  ]);
  ...
  await logEvent({
    ...
    ipAddress,
    userAgent,
    metadata: { docHashAfter: newHash, signerName: recipient.full_name },
  });
  ...
  if (parseInt(remaining[0].cnt) === 0) {
    ...
    // No parallel fire-and-forget import, no separate emailQueue plain-link jobs.
